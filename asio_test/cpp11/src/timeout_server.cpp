...
public:
    tcp_session(tcp::socket socket, channel &ch)
        ...
        // point whenever the output queue is empty. This ensures that the
        // output actor stays asleep until a message is put into the queue.
...
private:
    ...
    void read_line() {
        ...
        boost::asio::async_read_until(
                ...
                [this, self](const boost::system::error_code &error,
                             std::size_t n) {
                    // Check if the session was stopped while the operation was
                    // pending.
                    if (stopped())
                        return;

                    if (!error) {
                        // Extract the newline-delimited message from the
                        // buffer.
                        ...
                        if (!msg.empty()) {
                            ...
                            // We received a heartbeat message from the client.
                            // If there's nothing else being sent or ready to be
                            // sent, send a heartbeat right back.
                            if (output_queue_.empty()) {
                                output_queue_.push_back("\n");

                                // Signal that the output queue contains
                                // messages. Modifying the expiry will wake the
                                // output actor, if it is waiting on the timer.
    ...
    void await_output() {
        ...
        non_empty_output_queue_.async_wait(
                [this, self](const boost::system::error_code & /*error*/) {
                    ...
                    if (output_queue_.empty()) {
                        // There are no messages that are ready to be sent. The
                        // actor goes to sleep by waiting on the
                        // non_empty_output_queue_ timer. When a new message is
                        // added, the timer will be modified and the actor will
                        // wake.
                        non_empty_output_queue_.expires_at(
    ...
    void check_deadline(steady_timer &deadline) {
        ...
        deadline.async_wait(
                [this, self,
                 &deadline](const boost::system::error_code & /*error*/) {
                    // Check if the session was stopped while the operation was
                    // pending.
                    if (stopped())
                        return;

                    // Check whether the deadline has passed. We compare the
                    // deadline against the current time since a new
                    // asynchronous operation may have moved the deadline before
                    // this actor had a chance to run.
...
public:
    server(boost::asio::io_context &io_context,
        ...
        channel_.join(std::make_shared<udp_broadcaster>(io_context_,
                                                        broadcast_endpoint));
...
private:
    void accept() {
        acceptor_.async_accept([this](const boost::system::error_code &error,
                                      tcp::socket socket) {
            if (!error) {
                std::make_shared<tcp_session>(std::move(socket), channel_)
                        ->start();
            }
...
int main(int argc, char *argv[]) {
    try {
        ...
        if (argc != 4) {
            std::cerr << "Usage: server <listen_port> <bcast_address> "
                         "<bcast_port>\n";
            return 1;
        }
        ...
        udp::endpoint broadcast_endpoint(boost::asio::ip::make_address(argv[2]),
                                         atoi(argv[3]));
